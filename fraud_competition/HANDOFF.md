...
| make_cards_html.py | `python make_cards_html.py`: builds both handouts from materials/technique_cards.md: materials/technique_cards.html (no code) and materials/technique_cards_with_code.html (a closed "Hint: show code" dropdown per card). Edit the .md, then rebuild. Deployed on GitHub Pages from `main` as https://kartikgupta98.github.io/dl-worksheets/flagged_or_fraud_techniques.html and .../flagged_or_fraud_techniques_code.html: copy the rebuilt files to those names in `main` and push. |
